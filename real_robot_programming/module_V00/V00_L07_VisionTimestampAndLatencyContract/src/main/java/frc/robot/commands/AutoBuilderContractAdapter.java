...
public final class AutoBuilderContractAdapter {
  ...
  public CommandCreationResult createPathCommandResult(AutonomousStartContext context) {
    return createPathCommandResult(context, null);
  }

  /** Creates one fresh event-marker path command after the inherited preparation preview. */
  public CommandCreationResult createEventPathCommandResult(AutonomousStartContext context) {
    return createPathCommandResult(context, AutonomousEventId.LEARNING_EVENT);
  }

  private CommandCreationResult createPathCommandResult(
      AutonomousStartContext context, AutonomousEventId eventId) {
    ...
    try {
      PathPlannerPath canonicalPath =
          eventId == null
              ? pathPlannerTrajectoryAdapter.createCanonicalPath()
              : pathPlannerTrajectoryAdapter.createCanonicalEventPath();
      PathPlannerPath executionPath =
          eventId == null
              ? PathPlannerExecutionPathFactory.createExecutionPath(
                  canonicalPath, context.fieldVariant(), context.alliance())
      ...
      latchFault(
          eventId == null
              ? "AutoBuilder path command creation failed"
              : "AutoBuilder event path command creation failed",
          failure);
      return new CommandCreationResult(
          CommandCreationStatus.FAULTED, Optional.empty(), firstFaultReason);
    }
