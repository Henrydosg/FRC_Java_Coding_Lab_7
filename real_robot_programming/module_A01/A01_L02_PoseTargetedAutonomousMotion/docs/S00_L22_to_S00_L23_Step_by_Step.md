...
### Step 4 - Correct the verified drive ratio

- Step: 4
- Objective: replace the inherited disproven ratio assumption with measured hardware truth.
- Why: distance and velocity scale must match the physical drivetrain before odometry is meaningful.
- Action: changed the single drive-ratio Constants authority from `7.846153846153847` to `6.75` and updated only direct old-value test fixtures.
...
- Verification: PASS - hardware documentation, repeated FR raw `RotorPosition` over 20 wheel rotations, configuration tests, and later floor validation.
