...
public final class RobotTelemetry {
  ...
  private final Optional<ElevatorSubsystem> elevatorSubsystem;
  private final Optional<ElevatorTelemetryFacade> elevatorTelemetryFacade;
  ...
  /**
   * Creates the runtime telemetry coordinator with optional mechanism diagnostics including
   * Elevator position/reference observation.
   *
   * @param elevatorSubsystem immutable Elevator observation source
   * @param elevatorTelemetryFacade Elevator observation publisher
   */
  public RobotTelemetry(
      SwerveSubsystem swerveSubsystem,
      SwerveTelemetryFacade swerveTelemetryFacade,
      ...
      ElevatorTelemetryFacade elevatorTelemetryFacade) {
    ...
    if ((elevatorSubsystem == null) != (elevatorTelemetryFacade == null)) {
      throw new IllegalArgumentException(
          "Elevator observation source and facade must be supplied together");
    }
    this.elevatorSubsystem = Optional.ofNullable(elevatorSubsystem);
    this.elevatorTelemetryFacade = Optional.ofNullable(elevatorTelemetryFacade);
  }
  ...
  public void periodic() {
    ...
    if (elevatorSubsystem.isPresent()) {
      elevatorTelemetryFacade
          .orElseThrow()
          .publish(elevatorSubsystem.orElseThrow().getObservation());
    }
