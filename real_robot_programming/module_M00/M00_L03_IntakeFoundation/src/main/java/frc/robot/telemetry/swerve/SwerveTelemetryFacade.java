...
public final class SwerveTelemetryFacade implements AutoCloseable {
  ...
  private final DoublePublisher qualifiedHandoffCountPublisher;
  private final DoublePublisher lastQualifiedHandoffTimestampSecondsPublisher;
  private final DoublePublisher acceptedFusionCountPublisher;
  private final DoublePublisher lastAcceptedFusionTimestampSecondsPublisher;
  ...
  SwerveTelemetryFacade(
    ...
    NetworkTable visionFusionTable = swerveTable.getSubTable("VisionFusion");
    qualifiedHandoffCountPublisher =
        visionFusionTable.getDoubleTopic("QualifiedHandoffCount").publish();
    lastQualifiedHandoffTimestampSecondsPublisher =
        visionFusionTable
            .getDoubleTopic("LastQualifiedHandoffTimestampSeconds")
            .publish();
    acceptedFusionCountPublisher =
        visionFusionTable.getDoubleTopic("AcceptedFusionCount").publish();
    lastAcceptedFusionTimestampSecondsPublisher =
        visionFusionTable
            .getDoubleTopic("LastAcceptedFusionTimestampSeconds")
            .publish();
  }
  ...
  public void publish(SwerveObservation observation) {
    publishSwerveObservation(observation);
  }

  /** Publishes Swerve state and immutable vision-fusion evidence without controlling behavior. */
  public void publish(
      SwerveObservation observation,
      VisionFusionObservation visionFusionObservation) {
    Objects.requireNonNull(observation, "observation");
    VisionFusionObservation requiredVisionFusionObservation =
        Objects.requireNonNull(visionFusionObservation, "visionFusionObservation");

    publishSwerveObservation(observation);
    publishVisionFusion(requiredVisionFusionObservation);
  }

  private void publishSwerveObservation(SwerveObservation observation) {
    Objects.requireNonNull(observation, "observation");
  ...
  private void publishVisionFusion(VisionFusionObservation observation) {
    qualifiedHandoffCountPublisher.set(observation.qualifiedHandoffCount());
    lastQualifiedHandoffTimestampSecondsPublisher.set(
        observation.lastQualifiedHandoffTimestampSeconds());
    acceptedFusionCountPublisher.set(observation.acceptedFusionCount());
    lastAcceptedFusionTimestampSecondsPublisher.set(
        observation.lastAcceptedFusionTimestampSeconds());
  }
  ...
  public void close() {
    ...
    qualifiedHandoffCountPublisher.close();
    lastQualifiedHandoffTimestampSecondsPublisher.close();
    acceptedFusionCountPublisher.close();
    lastAcceptedFusionTimestampSecondsPublisher.close();
    field.close();
  }
