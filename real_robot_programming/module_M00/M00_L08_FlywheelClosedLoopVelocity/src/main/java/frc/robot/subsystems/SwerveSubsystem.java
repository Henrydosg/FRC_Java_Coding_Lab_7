...
public class SwerveSubsystem extends SubsystemBase {
  ...
  private static final double VISION_ESTIMATOR_HISTORY_SECONDS = 1.5;
  ...
  private final SwerveKinematics measuredSpeedKinematics = new SwerveKinematics();
  ...
  private boolean estimatorContinuityValid;
  private double latestEstimatorUpdateTimestampSeconds = Double.NaN;
  private double visionResetBarrierTimestampSeconds = Double.NEGATIVE_INFINITY;
  private double lastAcceptedVisionTimestampSeconds = Double.NEGATIVE_INFINITY;
  private long qualifiedVisionHandoffCount;
  private double lastQualifiedVisionHandoffTimestampSeconds = Double.NaN;
  private long acceptedVisionFusionCount;
  private double lastAcceptedVisionFusionTimestampSeconds = Double.NaN;
  ...
  /** Immutable validated localization inputs for one known-field-pose reset. */
  private record LocalizationResetSnapshot(
      Pose2d requestedPose,
      Rotation2d acceptedFieldHeading,
  ...
  public void periodic() {
    ...
    latestObservation =
        Optional.of(
            new SwerveObservation(
                ...
                toEstimatedPoseObservation(poseMeasurementSampleValid)));
    ...
    if (!DriverStation.isEnabled() && productionIntentArmed) {
      stop();
      return;
    }
  ...
  /** Returns immutable evidence for qualified vision handoffs and accepted estimator fusions. */
  public VisionFusionObservation getVisionFusionObservation() {
    return new VisionFusionObservation(
        qualifiedVisionHandoffCount,
        lastQualifiedVisionHandoffTimestampSeconds,
        acceptedVisionFusionCount,
        lastAcceptedVisionFusionTimestampSeconds);
  }
  ...
  /**
   * Returns a defensive snapshot of the current pose estimate.
   *
   * @return empty until the estimator initializes from one complete valid measurement sample
   */
  public Optional<Pose2d> getEstimatedPose() {
    return estimatedPose.map(SwerveSubsystem::copyPose);
  }

  /**
   * Admits one already-qualified field pose into the subsystem-owned estimator.
   *
   * <p>This is the only vision fusion entry point. All guards execute before the WPILib
   * estimator call, including estimator readiness, reset-barrier, future, history, and strict
   * timestamp-order checks. Rejected measurements do not alter odometry or estimator state.
   *
   * @param measurement immutable qualified field-relative vision measurement
   * @return true only when the estimator accepted the guarded admission
   */
  public boolean admitVisionMeasurement(QualifiedVisionMeasurement measurement) {
    if (measurement == null
        || !isFinitePose(measurement.fieldRobotPose())
        || measurement.quality().acceptance() != Acceptance.ACCEPTED
        || !Double.isFinite(measurement.measurementTimestampSeconds())) {
      return false;
    }

    double timestampSeconds = measurement.measurementTimestampSeconds();
    qualifiedVisionHandoffCount++;
    lastQualifiedVisionHandoffTimestampSeconds = timestampSeconds;
    ...
    if (!Double.isFinite(nowSeconds)
        ...
        || timestampSeconds > latestEstimatorUpdateTimestampSeconds
        || timestampSeconds <= visionResetBarrierTimestampSeconds
        || timestampSeconds <= lastAcceptedVisionTimestampSeconds
        || timestampSeconds
            < latestEstimatorUpdateTimestampSeconds - VISION_ESTIMATOR_HISTORY_SECONDS) {
      return false;
    }

    try {
      poseEstimator.addVisionMeasurement(measurement.fieldRobotPose(), timestampSeconds);
      estimatedPose = Optional.of(copyPose(poseEstimator.getEstimatedPosition()));
      lastAcceptedVisionTimestampSeconds = timestampSeconds;
      acceptedVisionFusionCount++;
      lastAcceptedVisionFusionTimestampSeconds = timestampSeconds;
      return true;
    } catch (RuntimeException ignored) {
      // A malformed or unavailable estimator state fails closed without affecting odometry.
      return false;
    }
  }

  /**
   * Resets both subsystem-owned localization trackers to one known field pose.
   *
   * <p>This is a localization-frame reset only. Physical gyro, drive-rotor, and CANcoder state
   * is never reset. The reset is accepted only while Disabled and after both localization trackers
   * have initialized from one complete valid measurement sample.
   *
   * @param requestedPose finite known field pose
   * @return true when odometry and estimated pose were reset together
   */
  public boolean resetKnownFieldPose(Pose2d requestedPose) {
    Optional<LocalizationResetSnapshot> snapshot =
        createLocalizationResetSnapshot(requestedPose);
    if (snapshot.isEmpty()) {
      return false;
    }

    LocalizationResetSnapshot acceptedSnapshot = snapshot.orElseThrow();
    try {
      odometry.resetPosition(
          acceptedSnapshot.acceptedFieldHeading(),
          acceptedSnapshot.modulePositions(),
          acceptedSnapshot.requestedPose());
      poseEstimator.resetPosition(
          acceptedSnapshot.acceptedFieldHeading(),
          acceptedSnapshot.modulePositions(),
          acceptedSnapshot.requestedPose());
    } catch (RuntimeException failure) {
      // Do not expose a partial reset. The existing continuity recovery will rebuild both
      // trackers from the previously exposed poses and the next valid sensor baseline.
      odometryContinuityValid = false;
      estimatorContinuityValid = false;
      return false;
    }

    currentPose = Optional.of(copyPose(acceptedSnapshot.requestedPose()));
    estimatedPose = Optional.of(copyPose(acceptedSnapshot.requestedPose()));
    odometryContinuityValid = true;
    estimatorContinuityValid = true;
    establishVisionResetBarrier();
    return true;
  }
   ...
   *
   * <p>Null or nonfinite requests fail closed through the existing all-module stop path.
   *
  ...
  public void acceptChassisSpeeds(ChassisSpeeds chassisSpeeds) {
    if (!isFiniteChassisSpeeds(chassisSpeeds)) {
      stop();
      return;
    }

    ChassisSpeeds acceptedSpeeds = chassisSpeeds;
   ...
   *
   * <p>Null or nonfinite requests, and invalid heading data, cause a full stop and disarm
   * production intent. No zero-heading substitution or robot-relative fallback is performed.
   *
  ...
  public void acceptFieldRelativeChassisSpeeds(ChassisSpeeds fieldRelativeSpeeds) {
    if (!isFiniteChassisSpeeds(fieldRelativeSpeeds)) {
      stop();
      return;
    }

    ChassisSpeeds acceptedSpeeds = fieldRelativeSpeeds;
  ...
  /**
   * Returns the instantaneous measured robot-relative chassis velocity.
   *
   * <p>The result is derived only from one complete FL, FR, BL, BR module-velocity and
   * module-angle observation. It does not use gyro data, field-heading capture, requested
   * chassis intent, or final commanded module states.
   *
   * @return measured robot-relative speeds when all four module measurements are valid; empty
   *     otherwise
   */
  public Optional<ChassisSpeeds> getMeasuredRobotRelativeSpeeds() {
    ...
    SwerveObservation observation = latestObservation.orElseThrow();
    SwerveObservation.ModuleObservation[] modules = {
      observation.frontLeft(),
      observation.frontRight(),
      observation.backLeft(),
      observation.backRight()
    };
    for (SwerveObservation.ModuleObservation module : modules) {
      if (!moduleHealthy(module)
          || !Double.isFinite(module.driveVelocityRotationsPerSecond())
          || !Double.isFinite(module.encoderAbsolutePositionRotations())) {
        return Optional.empty();
      }
    }
  ...
  private boolean updateOdometry(
    ...
    boolean odometryInitializedThisCycle = false;
    if (odometry == null) {
      ...
      odometryContinuityValid = true;
      odometryInitializedThisCycle = true;
    }

    boolean estimatorInitializedThisCycle = false;
    if (poseEstimator == null) {
      establishVisionResetBarrier();
      Pose2d initialPose = new Pose2d(0.0, 0.0, acceptedHeading);
      poseEstimator =
          new SwerveDrivePoseEstimator(
              ...
              modulePositions,
              initialPose);
      estimatedPose = Optional.of(copyPose(initialPose));
      estimatorContinuityValid = true;
      estimatorInitializedThisCycle = true;
    }

    if (!odometryInitializedThisCycle && !odometryContinuityValid) {
      ...
      odometry =
          new SwerveDriveOdometry(
              ...
              heldPose);
      odometryContinuityValid = true;
    } else if (!odometryInitializedThisCycle) {
      currentPose = Optional.of(copyPose(odometry.update(acceptedHeading, modulePositions)));
    }

    if (!estimatorInitializedThisCycle && !estimatorContinuityValid) {
      Pose2d heldEstimatedPose = estimatedPose.orElseThrow();
      establishVisionResetBarrier();
      poseEstimator =
          new SwerveDrivePoseEstimator(
              odometryKinematics,
              ...
              heldEstimatedPose);
      estimatorContinuityValid = true;
    } else if (!estimatorInitializedThisCycle) {
      estimatedPose =
          Optional.of(copyPose(poseEstimator.update(acceptedHeading, modulePositions)));
    }
    latestEstimatorUpdateTimestampSeconds = Timer.getFPGATimestamp();
    return true;
  }
  ...
  private Optional<LocalizationResetSnapshot> createLocalizationResetSnapshot(
      Pose2d requestedPose) {
    if (DriverStation.isEnabled()
        || !isFinitePose(requestedPose)
        || !gyroSnapshotInitialized
        || odometry == null
        || poseEstimator == null
        || currentPose.isEmpty()
        || estimatedPose.isEmpty()
    ...
    SwerveObservation.GyroObservation gyroObservation = observation.gyro();
    if (!gyroObservation.connected()
        || !gyroObservation.configurationHealthy()
        || !Double.isFinite(gyroObservation.yawDegrees())) {
      return Optional.empty();
    }

    Optional<Rotation2d> acceptedFieldHeading =
        fieldHeadingFromRawYaw(gyroObservation.yawDegrees());
    if (acceptedFieldHeading.isEmpty()) {
      return Optional.empty();
    }
    ...
    return Optional.of(
        new LocalizationResetSnapshot(
            copyPose(requestedPose),
            acceptedFieldHeading.orElseThrow(),
            copyModulePositions(modulePositions)));
  }
  ...
  private static SwerveModulePosition[] copyModulePositions(
      SwerveModulePosition[] modulePositions) {
    SwerveModulePosition[] copiedPositions = new SwerveModulePosition[modulePositions.length];
    for (int moduleIndex = 0; moduleIndex < modulePositions.length; moduleIndex++) {
      SwerveModulePosition modulePosition = modulePositions[moduleIndex];
      copiedPositions[moduleIndex] =
          new SwerveModulePosition(
              modulePosition.distanceMeters,
              new Rotation2d(modulePosition.angle.getRadians()));
    }
  ...
  private Optional<SwerveObservation.EstimatedPoseObservation> toEstimatedPoseObservation(
      boolean measurementSampleValid) {
  ...
  private static boolean isFiniteChassisSpeeds(ChassisSpeeds chassisSpeeds) {
    return chassisSpeeds != null
        && Double.isFinite(chassisSpeeds.vxMetersPerSecond)
  ...
  private Optional<Rotation2d> currentFieldHeading() {
    ...
    return fieldHeadingFromRawYaw(gyroInputs.yawDegrees);
  }

  private Optional<Rotation2d> fieldHeadingFromRawYaw(double rawYawDegrees) {
    if (!fieldHeadingReferenceValid || !Double.isFinite(rawYawDegrees)) {
      return Optional.empty();
    }
    double fieldHeadingDegrees =
        MathUtil.inputModulus(
            rawYawDegrees - capturedRawYawDegrees,
