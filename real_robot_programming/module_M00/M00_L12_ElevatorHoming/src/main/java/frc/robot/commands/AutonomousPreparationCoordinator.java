...
public final class AutonomousPreparationCoordinator {
  ...
  private synchronized void markRunning(long attemptId) {
    ...
    latestObservation =
        copyObservation(
            ...
            returnedCommandForCurrentRoutine(),
            true);
  }
  ...
  private synchronized void markHolding(long attemptId) {
    ...
    if (actions.isAdapterFaulted()
      ...
      transitionToFaulted(
          routineIdForCurrentObservation(),
          preparedAttempt == null
              ? Optional.empty()
              : Optional.ofNullable(preparedAttempt.alliance()));
      return;
  ...
  private synchronized void finishExecution(long attemptId, boolean interrupted) {
    ...
    latestObservation =
        copyObservation(
            state, false, reason, returnedCommandForCurrentRoutine(), false);
  }
  ...
  private static Routine toObservationRoutine(
    ...
    return switch (routineId) {
      ...
      case ONE_METER_WITH_EVENT -> Routine.ONE_METER_WITH_EVENT;
    };
  }

  private ReturnedCommand returnedCommandForCurrentRoutine() {
    return latestObservation.routine() == Routine.ONE_METER_WITH_EVENT
        ? ReturnedCommand.ONE_METER_WITH_EVENT
        : ReturnedCommand.ONE_METER_PATH;
  }

  private AutonomousRoutineFactory.AutonomousRoutineId routineIdForCurrentObservation() {
    return latestObservation.routine() == Routine.ONE_METER_WITH_EVENT
